...
def evaluate_episode(agent, env):
    ...
    while not done:
        ...
        _, action_idx, action_values = agent.act([state], [valid_ids], [past_actions], sample=False, return_all=True)

        cmd_id = min(past_states[state], len(action_idx) - 1)
        action_idx = action_idx[cmd_id]
        # action_values = action_values[cmd_id]
        action_str = valid_acts[action_idx]

        past_actions.append(action_str)
...
def train(agent, eval_env, envs, max_steps, update_freq, eval_freq, checkpoint_freq, log_freq):
    ...
    past_actions = [[] for _ in range(len(states))]

    for step in range(1, max_steps + 1):
        action_ids, action_idxs, _ = agent.act(states, valid_ids, past_actions)
        ...
        for state, act, rew, next_state, valids, done, history in \
                zip(states, action_ids, rewards, next_states, next_valids, dones, past_actions):
            agent.observe(state, act, rew, next_state, valids, done, history)
            history.append(act)
...
def parse_args():
    ...
    parser.add_argument('--output_dir', default='logs/cross_deephome_history')
    parser.add_argument('--spm_path', default='../spm_models/unigram_8k.model')
    parser.add_argument('--rom_path', default='../games/deephome.z5')
    ...
    parser.add_argument('--memory_size', default=1000, type=int)
    parser.add_argument('--batch_size', default=8, type=int)
    parser.add_argument('--accummulate_step', default=8, type=int)
...
def start_redis():
    ...
    subprocess.Popen(['/nas/home/feiwang/redis/bin/redis-server', '--save', '\"\"', '--appendonly', 'no'])
    time.sleep(1)
